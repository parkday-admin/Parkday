...
export default function TodayCard({ trip, onOpen }) {
  ...
  return (
    <div className={styles.card} role="button" tabIndex={0} onClick={onOpen} onKeyDown={onActivateKey(onOpen)} aria-label={`${label}, day ${dayNum}. See full day.`}>
      <div className={styles.hdr}>
        <div className={styles.hdrLeft}>
          {icon && <i aria-hidden="true" className={`ti ${icon} ${styles.hdrIcon}`} />}
        ...
        <div className={styles.hdrStats}>
          ...
          <div className={styles.hdrStat}>
            ...
            <div className={styles.hdrStatVal} style={{ color: actual > planned ? 'var(--coral)' : 'var(--night)' }}>{fmt(actual)}</div>
          </div>
        </div>
      ...
      <div className={styles.body}>
        {dayEntries.length === 0 ? (
          <div className={styles.row}>
            ...
            <div className={styles.seeFull}>See full day <i aria-hidden="true" className="ti ti-arrow-right" /></div>
          </div>
        ) : (
          <>
            {upNext && (
              <div className={styles.row}>
                ...
                <div className={styles.upNext}>
                  <div className={styles.upNextIcon} style={{ background: categoryMeta(upNext.cat).bg }}>
                    <i aria-hidden="true" className={`ti ${categoryMeta(upNext.cat).icon}`} style={{ color: categoryMeta(upNext.cat).color }} />
                  </div>
